const Terms = {
  title: "개인정보 수집 및 이용 동의서(필수)",
  description:
    "(재)이노베이션 아카데미는 『개인정보 보호법』 제15조 등 관련 법령에 따라 서비스 이용자의 개인정보보호를 매우 중시하며, 서비스 제공에 반드시 필요한 개인정보의 수집⦁이용을 위하여 귀하의 동의를 받고자 합니다.",
  from: "(재)이노베이션 아카데미",
  content: {
    purpose: {
      title: "개인정보의 수집 및 이용 목적",
      content: ["어디있니 현재 위치 확인 서비스 제공"],
    },
    collectInfo: {
      title: "수집하는 개인정보 항목",
      content: [
        "인트라 로그인 아이디",
        "클러스터 출입 상태",
        "입실 시 현재 입실 한 클러스터",
        "출입카드 마지막 태그 시간",
      ],
    },
    retentionPeriod: {
      title: "개인정보의 보유 및 이용기간",
      content: [
        "3년",
        "보유기간 경과 및 보유목적 달성 시 지체 없이 파기합니다",
      ],
    },
    refusalAndConsequences: {
      title: "동의 거부 권리 및 동의 거부에 따른 불이익 내용 또는 제한사항",
      content: [
        "귀하는 개인정보 수집 및 이용에 대해 동의를 거부할 권리가 있습니다.",
        "필수항목에 대한 동의 거부 시 어디있니 서비스 제공이 제한됨을 알려드립니다.",
      ],
    },
  },
};

export default Terms;
